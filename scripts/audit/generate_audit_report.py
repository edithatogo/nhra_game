...
def generate_audit_report() -> None:
    ...
    if matrix_path.exists():
        ...
        with open(matrix_path, "r") as f:
            ...
            for row in reader:
                report.append(f"| {row['Feature']} | {row['Category']} | {row['Status']} | {row['Source']} |")
        report.append("\n")
    ...
    if orphans_path.exists():
        ...
        report.append(f"Identified {len(orphans)} instances of logic in archives that are not explicitly present in the current engine.")
    ...
    report.append("- **Bargaining Outside Option:** Diagrams show explicit 'Schedule K' and 'bailout' nodes which are currently simplified in the engine.")
    report.append("- **Audit Feedback Nuance:** Legacy versions had more granular 'coding effort' vs 'audit intensity' games that are now aggregated into a single `COMP` node.")
    report.append("- **Multi-Equilibrium Selection:** While implemented, the stability analysis across all discovered equilibria needs more robust visualization compared to legacy outputs.")
