...
def generate_parity_matrix() -> None:
    ...
    chatgpt_context_path = Path("context/origin_chatgpt_context.md")
    ...
    chatgpt_context = chatgpt_context_path.read_text() if chatgpt_context_path.exists() else ""
    ...
    current_features = {
        ...
        "pressure_index", "within4_from_pressure", "relative_risk",
        "step", "run_hybrid", "Params", "State", "apply_intervention",
        "nep_series", "input_cost_series", "one_way_sensitivity"
    }
    ...
    for diag, edges in visual_edges.items():
        for edge in edges:
    ...
    for src, tgt, desc in intended_influences:
        ...
        if src == "Audit" and "COMP" in current_features:
             status = "[Implemented]"
        elif src == "VFI" and "SHIFT" in current_features:
             status = "[Implemented]"
    ...
    for key in legacy_keys:
        found_in_legacy = any(key in str(f) for f in fingerprints.values())
