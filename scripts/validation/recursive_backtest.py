...
def main():
    ...
    if not historical_path.exists():
        print(
            f"Error: {historical_path} not found. Please run scripts/data/preprocess_historical.py first."
        )
        return
    ...
    engine = RecursiveBacktest(historical_data=df, train_window=5, test_window=1, seed=42)

    print("Starting Recursive Backtest (Rolling Horizon)...")
