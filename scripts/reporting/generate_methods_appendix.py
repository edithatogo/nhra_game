...
def generate_appendix(registry_path: Path, output_path: Path, references_path: Path):
    ...
    cite_101 = (
        "{Australian Institute of Health and Welfare, 2024 @AIHW_MyHospitals #101}"
        if 101 in bib_mgr.references
        else ""
    )
    content.append(
        f"- **Economic Spine:** Calibrated against historical NEP (IHACPA) and WPI (ABS) series {cite_101}."
    )
    ...
    content.append(
        f"This study adheres to the STRESS guidelines for reporting simulation studies {cite_103}."
    )
    ...
    for i, ref in enumerate(sorted_refs, 1):
        content.append(
            f"{i}. {ref.author}. ({ref.year}). *{ref.title}*. {ref.journal or ref.publisher or ''}. {ref.url or ref.doi or ''}"
        )
