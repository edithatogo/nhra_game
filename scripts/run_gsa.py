...
def main() -> None:
    ...
    parser.add_argument(
        "--samples", type=int, default=10, help="N samples (or trajectories for Morris)"
    )
    ...
    if args.method == "morris":
        ...
        df = run_morris_analysis(
            problem, model_func_for_salib, n_trajectories=args.samples, n_procs=args.procs
        )
    ...
    if args.method == "sobol":
        ...
        si = run_sobol_analysis(
            problem, model_func_for_salib, n_samples=args.samples, n_procs=args.procs
        )
