...
EDGE = re.compile(r'^\s*(?P<src>[A-Za-z0-9_]+)\s*->\s*(?P<dst>[A-Za-z0-9_]+)\s*(?:\[(?P<attrs>[^\]]+)\])?;\s*$')
...
def dot_to_mermaid(dot_text: str, direction: str = "LR") -> str:
    ...
    for raw in dot_text.splitlines():
        line = raw.strip()
        if not line or line.startswith("//") or line.startswith("digraph") or line.startswith("graph") or line.startswith("node") or line.startswith("edge") or line in ("{", "}"):
            continue
    ...
    out = []
    out.append('%%{init: {"theme":"base","flowchart":{"curve":"basis","nodeSpacing":55,"rankSpacing":75},"themeVariables":{"fontFamily":"Inter, Arial, sans-serif","fontSize":"14px","lineColor":"#444444"}}}%%')
