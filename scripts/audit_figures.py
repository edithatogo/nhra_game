    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name in ['matplotlib.pyplot', 'plotly.graph_objects', 'plotly.express', 'seaborn', 'matplotlib.figure']:
...
def audit_file(filepath: Path) -> List[Dict]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            if 'plot' in node.name.lower() or 'figure' in node.name.lower() or 'chart' in node.name.lower():
                findings.append({
                    "file": str(filepath),
                    "type": "definition",
                    "name": node.name,
