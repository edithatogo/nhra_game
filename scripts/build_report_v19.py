...
def main() -> None:
    ...
    md: list[str] = []
    md.append("# NHRA game-theory mechanism simulation — v19 report (2025–2030)")
    md.append("")
    md.append("**Date:** 21 Dec 2025  \
**Version:** v19 (Round 2 implementation)  \
**Purpose:** Provide decision-relevant outputs for RACMA policy positioning. This is a **stylised mechanism model** intended to clarify incentive structures and plausible directions of effect, not to generate point forecasts.")
    ...
    md.append("")
    md.append("### Figure 5. NEP vs input cost per NWAU (index)")
    md.append("")
    md.append(f"![NEP vs cost]({plots/'nep_vs_input_cost_index.png'})")
    md.append("")
    md.append("*Caption:* This figure compares NEP (annual $/NWAU index discipline) against a proxy input-cost index per NWAU. When input costs grow faster than NEP, the efficiency gap widens mechanically over time in the model.")
    md.append("")
    nep_cost_path = tables / "nep_cost_series.csv"
    if nep_cost_path.exists():
        nep_cost = pd.read_csv(nep_cost_path)
        md.append("**Table: NEP vs input cost series (excerpt).**")
        md.append("")
        md.append(md_table(nep_cost, max_rows=10))
        md.append("")
    md.append("## 2. Equilibria structure of stage games")
    md.append("")
    md.append("The model uses several 2×2 stage games (definition, bargaining, cost shifting, discharge coordination, governance integration, compliance). For transparency, v19 exports **all Nash equilibria** (pure + mixed) by year at the mean system state.")
    md.append("")
    ...
    # --- v19 additions: deterministic fixed-point + network effect proxy ---
    md.append("")
    md.append("## 5. Deterministic fixed-point and network effects")
    md.append("")
    fp = tables / "fixed_point_trajectory.csv"
    if fp.exists():
        md.append("### Figure 10. Fixed-point convergence")
        md.append("")
        md.append(f"![Fixed point]({plots/'fixed_point_convergence.png'})")
        md.append("")
        md.append("*Caption:* Deterministic iteration using equilibrium-selected strategies (noise disabled). Convergence indicates a steady-state attractor under the configured governance and incentive settings.")
        md.append("")
        md.append("### Figure 11. Pressure near steady-state")
        md.append("")
        md.append(f"![Pressure tail]({plots/'fixed_point_pressure_tail.png'})")
        md.append("")
    net_edges = tables / "network_edges_strong_corr.csv"
    if net_edges.exists():
        edges = pd.read_csv(net_edges)
        md.append("### Figure 12. Network effect proxy (correlation heatmap)")
        md.append("")
        md.append(f"![Network heatmap]({plots/'network_effect_correlation_heatmap.png'})")
        md.append("")
        md.append("*Caption:* A descriptive, model-internal proxy of how strategy shares co-move with system outcomes over time. This is not causal inference; it helps communicate where dynamics couple strongly.")
        md.append("")
        md.append("### Table 5. Strong strategy–outcome associations (|r| ≥ 0.35; excerpt)")
        md.append("")
        md.append(md_table(edges.round(3), max_rows=20))
        md.append("")
    md_text = "\n".join(md)
    md_path = repdir / "v19_report_20251221.md"
    md_path.write_text(md_text, encoding="utf-8")

    # HTML render (self-contained)
    html_path = repdir / "v19_report_20251221.html"
    import subprocess
