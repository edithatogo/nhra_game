...
@beartype
def qre_solver_jax(
    u_row: Float[jnp.ndarray, "m n"],
    u_col: Float[jnp.ndarray, "m n"],
    lam: float = 5.0,
    max_iter: int = 100,
    tol: float = 1e-6
) -> tuple[Float[jnp.ndarray, "m"], Float[jnp.ndarray, "n"]]:
    """
    ...
    Returns:
        A tuple of (row_strategy, col_strategy).
    """
    m, n = u_row.shape
    
    def logit_choice(utilities: jnp.ndarray) -> jnp.ndarray:
    ...
    def body_func(val):
        ...
        new_diff = jnp.max(jnp.abs(next_p - p)) + jnp.max(jnp.abs(next_q - q))
        return next_p, next_q, i + 1, new_diff
    ...
    def scan_body(val, _):
        ...
        next_p = logit_choice(exp_u_row)
        next_q = logit_choice(exp_u_col)
        return (next_p, next_q), None

    (p_final, q_final), _ = lax.scan(scan_body, (p0, q0), jnp.arange(max_iter))
...
def regret_min_solver_jax(
    ...
    final_logits, _ = lax.scan(scan_body, (p_logits, q_logits), jnp.arange(max_iter))
    
    return jax.nn.softmax(final_logits[0]), jax.nn.softmax(final_logits[1])
...
def discrete_nash_jax(
    ...
    # (i, j) is a pure NE if u_row[i,j] >= u_row[k,j] for all k
    # AND u_col[i,j] >= u_col[i,l] for all l
...
def solve_hierarchical_game_jax(
    macro_row_matrix: Float[jnp.ndarray, "m n"],
    macro_col_matrix: Float[jnp.ndarray, "m n"],
    ...
    """
    # 1. Solve for equilibria in all possible micro-game branches
    # Since we are in JAX, we can vmap this over the macro outcome grid.
    
    m, n = macro_row_matrix.shape
    
    # We'll use a simplified version: 
    # For each (i, j) in macro, get the micro-game payoffs and add them to macro.
    
    def get_micro_utility(i, j):
        u_micro_row, u_micro_col = micro_game_factory(i, j)
        p_micro, q_micro = qre_solver_jax(u_micro_row, u_micro_col, lam=lam)
    ...
    p_macro, q_macro = qre_solver_jax(effective_macro_row, effective_macro_col, lam=lam)
...
@beartype
def qre_3player_jax(
    u1: Float[jnp.ndarray, "m n k"], # Payoff for P1 (m actions) given P2 (n) and P3 (k)
    u2: Float[jnp.ndarray, "m n k"], # Payoff for P2
    u3: Float[jnp.ndarray, "m n k"], # Payoff for P3
    lam: float = 5.0,
    max_iter: int = 100,
    tol: float = 1e-6
) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    def scan_body(probs, _):
        ...
        return (next_p1, next_p2, next_p3), None

    p1_0 = jnp.ones(m) / m
    p2_0 = jnp.ones(n) / n
    p3_0 = jnp.ones(k) / k
    
    (pf1, pf2, pf3), _ = lax.scan(scan_body, (p1_0, p2_0, p3_0), jnp.arange(max_iter))
