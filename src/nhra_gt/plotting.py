...
def plot_trajectory(
    ...
    warnings.warn("plot_trajectory is deprecated, use nhra_game_theory.visualization.trajectories instead", DeprecationWarning, stacklevel=2)
    config = PlotConfig()
...
def plot_strategy_heatmap(freq: pd.DataFrame, outpath: Path) -> None:
    warnings.warn("plot_strategy_heatmap is deprecated, use nhra_game_theory.visualization.distributional instead", DeprecationWarning, stacklevel=2)
    config = PlotConfig()
...
def tornado_from_rankcorr(
    ...
    warnings.warn("tornado_from_rankcorr is deprecated, use nhra_game_theory.visualization.sensitivity instead", DeprecationWarning, stacklevel=2)
    config = PlotConfig()
