...
class BibliographyManager:
    ...
    def to_ris(self) -> str:
        ...
        for ref in self.references.values():
            ...
            if ref.journal: lines.append(f"JO  - {ref.journal}")
            if ref.volume: lines.append(f"VL  - {ref.volume}")
            if ref.issue: lines.append(f"IS  - {ref.issue}")
            if ref.pages:
                sp, ep = ref.pages.split("-") if "-" in ref.pages else (ref.pages, "")
                lines.append(f"SP  - {sp}"); lines.append(f"EP  - {ep}")
            if ref.doi: lines.append(f"DO  - {ref.doi}")
            if ref.url: lines.append(f"UR  - {ref.url}")
            lines.append("ER  - ")
            lines.append("")
        return "\n".join(lines)
    ...
    def to_enw(self) -> str:
        ...
        for ref in self.references.values():
            ...
            if ref.journal: lines.append(f"%J {ref.journal}")
            if ref.volume: lines.append(f"%V {ref.volume}")
            if ref.issue: lines.append(f"%N {ref.issue}")
            if ref.pages: lines.append(f"%P {ref.pages}")
            if ref.doi: lines.append(f"%R {ref.doi}")
            if ref.url: lines.append(f"%U {ref.url}")
    ...
    def to_bibtex(self) -> str:
        ...
        for ref in self.references.values():
            ...
            if ref.journal: lines.append(f"  journal = {{{ref.journal}}},")
            if ref.volume: lines.append(f"  volume = {{{ref.volume}}},")
            if ref.issue: lines.append(f"  number = {{{ref.issue}}},")
            if ref.pages: lines.append(f"  pages = {{{ref.pages}}},")
            if ref.doi: lines.append(f"  doi = {{{ref.doi}}},")
            if ref.url: lines.append(f"  url = {{{ref.url}}}")
            lines.append("}")
            lines.append("")
        return "\n".join(lines)
