...
def tests(session: nox.Session) -> None:
    ...
    session.install("pytest", "pytest-mock", "hypothesis", "hypothesis-auto", "icontract")
...
def coverage(session: nox.Session) -> None:
    ...
    session.install("-e", ".[dev,opt]")
    session.install("pytest", "pytest-cov", "pytest-mock", "hypothesis>=6.112.0", "importlib_metadata>=8.0.0", "hypothesis-auto", "icontract", "beartype", "typeguard")
    session.run(
        "pytest", 
        "--cov=src", 
        "--cov=scripts",
...
def load(session: nox.Session) -> None:
    ...
    session.run("locust", "-f", "tests/load/locustfile.py", "--headless", "--run-time", "1m", env=PYTHONPATH)