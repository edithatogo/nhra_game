...
1. **Convergence Guard:** Add a statistical convergence indicator to the UI to warn users when low MC samples (Lite mode) might be unstable.
2. **Signalling Game Visualization:** Explicitly parameterize and plot the signalling/transparency game logic.

### Low Priority (Hygiene)

1. **Legacy Ref Cleanup:** Remove metadata-only references to `v1`, `v2`, `v3` in the dashboard script to reduce forensic noise.
