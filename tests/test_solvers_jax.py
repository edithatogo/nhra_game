...
import numpy as np
import pytest
from nhra_gt.solvers_jax import qre_solver_jax, regret_min_solver_jax, discrete_nash_jax, qre_3player_jax
...
def test_qre_solver_basic():
    ...
    p, q = qre_solver_jax(u_row, u_col, lam=10.0)
...
def test_qre_differentiability():
    def get_eq_prob(eg):
        ...
        p, q = qre_solver_jax(u_row, u_col, lam=2.0)
...
def test_regret_min_basic():
    ...
    p, q = regret_min_solver_jax(u_row, u_col)
...
def test_qre_3player_basic():
    ...
    p1, p2, p3 = jax.jit(lambda x: qre_3player_jax(x, x, x, lam=10.0))(u)
