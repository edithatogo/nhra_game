...
from nhra_game_theory.sensitivity import (
    generate_sensitivity_summary, plot_sobol_indices, plot_sobol_heatmap,
    run_psa, get_salib_problem
)
...
def test_generate_sensitivity_summary_logic(tmp_path):
    ...
    pd.DataFrame({
        "mu_star": [0.5, 0.2],
        "sigma": [0.1, 0.05]
    }, index=["param1", "param2"]).to_csv(morris_csv)
