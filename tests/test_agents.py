...
def test_lhn_utility_ramping_penalty():
    ...
    u_base = lhn_utility(
        ...
        revenue=jnp.array(100.0),
        cost=jnp.array(80.0),
        is_shifting=False,
        delta_target_capacity=jnp.array(0.0),
        weights=weights,
    )

    # Case 2: Ramping (pressure = 1.2)
    u_high = lhn_utility(
        pressure=jnp.array(1.2),
        revenue=jnp.array(100.0),
        cost=jnp.array(80.0),
        is_shifting=False,
        delta_target_capacity=jnp.array(0.0),
...
def test_state_utility_vfi():
    ...
    u_base = state_utility(
        fiscal_gap=jnp.array(0.0), lhn_performance=jnp.array(50.0), weights=weights
    )

    # Case 2: Positive fiscal gap
    u_gap = state_utility(
        fiscal_gap=jnp.array(10.0), lhn_performance=jnp.array(50.0), weights=weights
    )
