...
import pytest
from nhra_gt.domain.state import ParamsJax, StateJax, MetricsJax, LhnState
from nhra_gt.engine_jax import step_jax, baseline_state_jax
...
def test_heuristic_agent_venue_choice():
    ...
    # Case A: Low pressure
    s_low = State(
        year=2025, month=1, pressure=0.9, occupancy=0.8, offload_min=10.0,
        within4=0.8, effective_cth_share=0.45, efficiency_gap=0.05,
        discharge_delay=1.0, political_capital=1.0, system_mode=SystemMode.NORMAL,
        target_capacity=1.0, current_capacity=1.0, equity_index=1.0,
        reconciliation_balance=0.0, bailout_expectation=0.0, coding_intensity=1.0,
        reputation_score=1.0, auditor_suspicion=0.0, audit_pressure_active=0.5
    )
