...
from nhra_gt.engine_jax import ParamsJax, baseline_state_jax, lhn_step_jax
...
def test_hierarchical_vectorization_smoke():
    ...
    # Vectorized LHN step
    vmap_lhn_step = jax.vmap(
        lambda k, d, o: lhn_step_jax(
...
def test_state_delegation_impact():
    ...
    res_loose = lhn_step_jax(
        s, p, strategies, jnp.array(1.0), 1.0 / 12.0, jnp.array(0.0), jnp.array(1.5), jnp.array(1.0)
    )
    # LHN with tight target
    res_tight = lhn_step_jax(
        s,
        p,
        strategies,
        jnp.array(1.0),
